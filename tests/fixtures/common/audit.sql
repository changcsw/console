...
INSERT INTO platform.audit_logs (actor_id, action, resource_type, resource_id, env, detail_json, created_at)
...
WHERE NOT EXISTS (SELECT 1 FROM platform.audit_logs WHERE action='game.update' AND resource_id='g_1001');

INSERT INTO platform.audit_logs (actor_id, action, resource_type, resource_id, env, detail_json, created_at)
SELECT 1, 'game_channel.hide', 'game_channel', 'gc_2002', 'sandbox',
       '{"summary":"隐藏渠道实例 gc_2002"}'::jsonb,
       TIMESTAMPTZ '2026-06-17 08:00:00+00'
WHERE NOT EXISTS (SELECT 1 FROM platform.audit_logs WHERE action='game_channel.hide' AND resource_id='gc_2002');
...
WHERE NOT EXISTS (SELECT 1 FROM platform.audit_logs WHERE action='sync.execute' AND resource_id='g_1001' AND env='production');
...
WHERE NOT EXISTS (SELECT 1 FROM platform.audit_logs WHERE action='payment_route.update' AND resource_id='pr_3003');
...
WHERE NOT EXISTS (SELECT 1 FROM platform.audit_logs WHERE action='fx.apply' AND resource_id='run_4004');
