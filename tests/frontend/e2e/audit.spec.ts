...
test("审计列表渲染行/动作标签/资源标识/摘要，且 production 高亮", async ({ page }) => {
  ...
  // production 高亮（danger 标签）
  await expect(table.locator(".el-tag--danger")).toBeVisible();
...
test("提交式过滤：选择动作后点查询才下发请求", async ({ page }) => {
  ...
  await page.getByPlaceholder("动作").click();
...
test("详情抽屉 update：before/after 左右对照 + changed 高亮 + request 折叠", async ({ page }) => {
  ...
  await page.screenshot({ path: "../../tests/frontend/screenshots/audit-detail-update.png", fullPage: true });
});
...
test("详情抽屉 create：仅 after 单列", async ({ page }) => {
  ...
  const drawer = page.locator(".el-drawer");
  await expect(drawer.getByText("新游戏")).toBeVisible();
});
...
test("详情抽屉 delete：仅 before 单列", async ({ page }) => {
  ...
  const drawer = page.locator(".el-drawer");
  await expect(drawer.getByText("旧游戏")).toBeVisible();
});

test("密文字段以 ****** 展示，绝不出现明文", async ({ page }) => {
  await setup(page);
  await gotoAudit(page);
  await page.getByText("更新 google 账号认证配置").click();
  const drawer = page.locator(".el-drawer");
  await expect(drawer.getByText("******").first()).toBeVisible();
  await expect(drawer).not.toContainText("masked");
});
...
test("审计列表视觉基线", async ({ page }) => {
  await setup(page);
  await gotoAudit(page);
  await expect(page.locator(".el-table").getByText("game.update")).toBeVisible();
