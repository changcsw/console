...
import type { FeaturePluginTemplate } from "@/api/modules/featurePlugins";

// 插件参数模板四件套的编辑态草稿与提交载荷之间的纯转换。
// 与后端插件模板校验同口径做前置提示，服务端仍会二次强制校验。

/** 抽屉里的模板四件套编辑态。validationRulesJson 用 JSON 文本直填，提交前解析。 */
export interface PluginTemplateDraft {
...
export interface PluginTemplateQuartetPayload {
...
export function emptyDraft(): PluginTemplateDraft {
  return { fields: [emptyField(10)], secretFields: [], fileFields: [], rulesText: "{}" };
}

export function draftFromTemplate(tpl: FeaturePluginTemplate): PluginTemplateDraft {
  return {
...
export function fieldKeys(draft: PluginTemplateDraft): string[] {
 ...
 */
export function draftToPayload(draft: PluginTemplateDraft): { payload: PluginTemplateQuartetPayload } | { error: string } {
