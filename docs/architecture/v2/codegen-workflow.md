...
| 🟦🧪 **后端测试工程师**           | **Codex 5.3 Medium** | 后端单元测试 + 接口场景矩阵 manifest 并运行                        |
...
| 🟩🧪 **前端测试工程师**           | **Codex 5.3 Medium** | 前端 vitest 组件测试 + Playwright UI 用例（mock 契约）并运行       |
| 🟪 **测试专家**                | **Opus 4.8 High**    | 前后端汇合后的集成 / 系统测试、跨栈 e2e、全量回归、契约对账                   |
| 🟧 **高级全栈工程师**             | **Opus 4.8 High**    | 仅在集成阶段修复测试专家发现的问题（跨前后端改代码）                          |
| ✅ **功能验收师**                | **Composer 2.5**     | 端到端功能验收，对照操作主线与 compact 业务规则                        |
...
启动各角色 subagent 时，通过 Task 工具的 `model` 参数指定模型。
