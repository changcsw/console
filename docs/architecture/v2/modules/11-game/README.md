...
> 凡 00 公共已定义的：env 模型、统一响应包络、统一错误码、分页、鉴权与权限码、审计、命名与默认值兜底，本文不再重复，仅在需要落地时引用。
> 若本文与 00 公共冲突，以 00 公共为准；本文只在公共基座上**追加**游戏主数据模块的私有约定。

涉及的真实数据表：`games`、`game_markets`、`game_legal_links`（DDL 见 `services/admin-api/migrations/000001_init.up.sql`，本文按已锁定决策 D1 在其上叠加 `env` 列与新唯一键）。
...
「游戏主数据」是整个发行后台的**根聚合**与一切下游配置的挂载点。它对应领域层 `internal/domain/game`（见 01 结构 §4）的 `Game` 聚合根，承载一个游戏在**某个 env** 下的：
...
- 因此本模块是**最先要落地**的业务模块；它的 env 模型、唯一键、ID 生成规则会被所有下游沿用。

### 1.3 模块职责（做什么）

1. 在**当前运行环境（env）**下创建 / 列表 / 查看 / 编辑游戏基础信息。
...
- **不**允许前端指定写入的 `env`：写操作一律落当前运行环境（00 公共 §2.1）。
...
`Game` 聚合根聚合三类实体/值对象，聚合边界 = 一个游戏在一个 env 下的全部主数据：

```text
Game (聚合根, 对应 games 行)
 ├─ 基础信息 (值对象集合)
 │    gameId / gameSecret / name / alias / iconUrl
 │    defaultMarketCode / status / env / createdAt / updatedAt
...
> 说明：以下 3 张表的「原始 DDL」来自 `migrations/000001_init.up.sql`；本文按 **D1** 在其上叠加 `env` 列与新唯一键（通过新增迁移文件实现，不改历史迁移，见 01 结构 §6）。每张表下方给出「v2 目标结构」。带 `env` 的列与「新唯一键」用 **【D1 新增】** 标注。
...
CREATE TABLE games (
  ...
  UNIQUE (env, game_id),    -- 【D1 新增】原 UNIQUE(game_id) 前置 env
  UNIQUE (env, alias)       -- 【D1 新增】alias 同 env 内唯一（见 §5.4）
);
```
...
| 列 | 类型 | 默认 | 约束 | 说明 |
| --- | --- | --- | --- | --- |
| `id` | BIGSERIAL | 自增 | PK | 内部主键，仅用于外键引用，**不对外暴露** |
| `env` | VARCHAR(16) | 无（取当前运行环境） | NOT NULL, CHECK in `develop/sandbox/production` | 【D1 新增】数据所属环境；写入时由服务端按 `APP_ENV` 注入，前端不可指定 |
| `game_id` | VARCHAR(64) | 无（自动生成） | NOT NULL, 唯一键组成 | 对外游戏标识，创建时生成，**不可变**（生成规则见 §5.1） |
| `game_secret` | VARCHAR(128) | 无（自动生成） | NOT NULL | 对外密钥，创建时生成，**响应一律脱敏**（§5.2、§6.1） |
| `name` | VARCHAR(128) | 无 | NOT NULL | 游戏展示名，可改 |
| `alias` | VARCHAR(64) | 无 | NOT NULL, `UNIQUE(env, alias)` | 简称/代号，同 env 内唯一（§5.4），可改但需校验唯一 |
...
- 原：`UNIQUE (game_id)` → v2：`UNIQUE (env, game_id)`。
- 追加：`UNIQUE (env, alias)`（落地 §5.4 的「alias 同 env 唯一」业务规则；DDL 原表无此键，由新增迁移补齐）。
...
CREATE TABLE game_markets (
  ...
  created_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE (env, game_id_ref, market_code)   -- 【D1 新增】原 UNIQUE(game_id_ref, market_code) 前置 env
);
```
...
| `env` | VARCHAR(16) | 当前运行环境 | NOT NULL, CHECK | 【D1 新增】必须与所属 `games.env` 一致（§5.7） |
| `game_id_ref` | BIGINT | 无 | NOT NULL, FK→`games(id)` | 所属游戏；被引用行与本行 env 必须一致（应用层保证，见 00 §2.2） |
...
唯一键变化（D1）：原 `UNIQUE (game_id_ref, market_code)` → v2 `UNIQUE (env, game_id_ref, market_code)`。
...
CREATE TABLE game_legal_links (
  ...
  created_at         TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  updated_at         TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
  UNIQUE (env, game_id_ref, scope_type, scope_value)  -- 【D1 新增】原 UNIQUE(game_id_ref, scope_type, scope_value) 前置 env
);
```
...
| `id` | BIGSERIAL | 自增 | PK | 内部主键 |
| `env` | VARCHAR(16) | 当前运行环境 | NOT NULL, CHECK | 【D1 新增】必须与所属 `games.env` 一致（§5.7） |
| `game_id_ref` | BIGINT | 无 | NOT NULL, FK→`games(id)` | 所属游戏；env 必须与被引用行一致 |
...
唯一键变化（D1）：原 `UNIQUE (game_id_ref, scope_type, scope_value)` → v2 `UNIQUE (env, game_id_ref, scope_type, scope_value)`。

### 3.4 跨表 env 一致性（D1 通用规则）

- 三表均带 `env`，且 `game_markets.env`、`game_legal_links.env` 必须等于其 `game_id_ref` 指向的 `games.env`。
- DB 层 FK 仅约束 `games(id)`，**不**自动约束 env 一致；由 `app` 服务层在写入时强制校验（00 §2.2、01 §4.2「仓储方法接收 ctx + env」）。
- 所有仓储查询都带 `WHERE env = $currentEnv`，避免跨环境串读。
...
| --- | --- | --- | --- | --- |
| `Environment` | `develop` / `sandbox` / `production` | `develop`（由 `APP_ENV` 决定，缺省 develop） | `games.env` / `game_markets.env` / `game_legal_links.env` | 运行/数据环境，写入不可由前端指定 |
...
- 规则：在当前 env 下，从基数 `100000` 起的**自增数字字符串**（与 `go_domain_api_draft.md` 示例 `"100001"` 一致），保证 `UNIQUE(env, game_id)`。
  - 实现建议：维护一个 per-env 序列（独立 sequence 或在 `domain/game` 内基于「当前 env 最大 game_id + 1」生成并在事务内校验唯一），冲突则重试。
  - 不同 env 各自从 `100000` 起独立计数（因唯一键已含 env）。
...
- `alias` 在**同一 env 内全局唯一**（落地为 `UNIQUE(env, alias)`，§3.1）。
- `POST` / `PATCH` 写入 alias 时，服务端先查重；冲突返回 `409 CONFLICT`（`message: "alias already exists"`，对齐 00 §7.2 示例）。
- 不同 env 间 alias 可重复（因唯一键含 env）。

### 5.5 legal scope 唯一与取值规则

- `(scopeType, scopeValue)` 在「同一游戏同一 env」内唯一（落地 `UNIQUE(env, game_id_ref, scope_type, scope_value)`，§3.3）。
- 取值约束（服务端校验）：
...
- 所有写操作的 `env` = 当前运行环境（`APP_ENV`），前端**不可指定**。
- `game_markets` / `game_legal_links` 写入时，`env` 必须与父 `games.env` 一致，由服务层强制注入与校验。
- 所有读取（列表/详情）默认 `WHERE env = 当前运行环境`，不跨 env 返回。
- `sandbox → production` 的跨 env 复制由 `sync`「同步」负责（section = `game/markets/legal`），本模块不直接处理跨 env 写。
...
> 统一遵循 00 公共 §7：前缀 `/api/admin`、`Authorization: Bearer <accessToken>`、`application/json; charset=utf-8`、字段 camelCase、时间 ISO-8601 UTC、统一响应包络、统一错误码、写操作落当前 env。下列每个写接口都标注权限码（00 §7.5）。
...
> 路径参数 `{gameId}` 指对外 `game_id`（如 `100001`），**不是**内部 `id`。服务端按 `(env, game_id)` 定位。
...
- **忽略**前端传入的 `gameId` / `gameSecret` / `env`。
...
- 行为：按 `(env, gameId)` 查询；不存在返回 `404 NOT_FOUND`。
...
- 权限：`game.write`。
- 仅允许修改基础信息字段；**不可改** `gameId` / `gameSecret` / `env`；markets 与 legalLinks 走各自专用接口。
...
- 行为：定位 `(env, gameId)`；逐字段 patch；`alias` 变更查重；`defaultMarketCode` 变更校验其已启用并同步 is_default；刷新 `updated_at`；写 `audit_logs`（`game.update`）。
...
> 遵循 01 结构 §4 分层：`domain/game`（纯逻辑）→ `app/command` `app/query`（编排）→ `infra/persistence/postgres`（pgx 仓储）→ `transport/http/games`（chi handler）。仓储保持窄（单聚合 CRUD + 必要查询），跨表编排放 app 层；所有仓储方法接收 `ctx` 与 `env`。
...
| 方法 | 类型 | 对应接口 | 说明 |
| --- | --- | --- | --- |
| `ListGames(ctx, env, ListGamesQuery)` | query | GET `/games` | 分页 + 过滤，返回摘要 |
| `GetGame(ctx, env, gameId)` | query | GET `/games/{gameId}` | 聚合详情，secret 脱敏 |
| `CreateGame(ctx, env, CreateGameCmd)` | command | POST `/games` | 生成 id/secret、建默认市场、审计 |
| `UpdateGame(ctx, env, UpdateGameCmd)` | command | PATCH `/games/{gameId}` | 部分更新基础信息 |
| `ReplaceMarkets(ctx, env, ReplaceMarketsCmd)` | command | PUT `/markets` | 全量覆盖 markets |
| `ReplaceLegalLinks(ctx, env, ReplaceLegalLinksCmd)` | command | PUT `/legal-links` | 全量覆盖 legalLinks |

依赖注入：`GameRepository`（仓储）、随机源（secret/id）、`AuditLogger`、`Clock`。

### 7.3 command / query 对象（`app/dto`）

- command（写）：`CreateGameCmd` / `UpdateGameCmd` / `ReplaceMarketsCmd` / `ReplaceLegalLinksCmd`，由 transport 层从请求 DTO 映射，携带 `actorId`、`env`。
...
```text
NextGameIDSeq(ctx, env) (int64, error)            // per-env 序列
ExistsAlias(ctx, env, alias, excludeGameID) (bool, error)
InsertGame(ctx, env, game) (Game, error)          // 同事务插 games + game_markets
GetGameByGameID(ctx, env, gameId) (Game, error)   // 聚合装配
ListGames(ctx, env, query) (items, total, error)
UpdateGame(ctx, env, gameId, patch) (Game, error)
ReplaceMarkets(ctx, env, gameId, markets) error   // 事务内 diff
ReplaceLegalLinks(ctx, env, gameId, links) error  // 事务内 diff
CountChannelsByMarket(ctx, env, gameId, marketCode) (int, error) // 删除保护（跨模块只读查询）
```
...
- 中间件链（01 §4 + 00 §7.5）：`recover` → 鉴权（Bearer）→ 权限（`game.read`/`game.write`）→ env 上下文注入 → handler → 审计。
...
| 公共能力（00/01） | 本模块如何落地 |
| --- | --- |
| env 模型（00 §2，D1） | 三表加 `env` + CHECK；唯一键前置 `env`；写落当前 env；读按 env 过滤 |
...
| 同步（00 §8 数据流 / `sync`） | 提供 section `game/markets/legal` 的可 diff 数据；本模块不执行跨 env 写 |
...
> 维度定义见 `03-testing.md §4`（S1 成功 / S2 鉴权401 / S3 权限403 / S4 校验失败 / S5 冲突 / S6 跨env / S7 审计 / S8 脱敏 / S9 分页 / S10 事务回滚）。`✓`=覆盖，`—`=不适用。后端 manifest：`tests/backend/scenarios/game.yaml`；前端 e2e：`tests/frontend/e2e/games.spec.ts`。
...
| PATCH `/api/admin/games/{gameId}` | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | — | ✓ | 默认市场必须已启用、alias 同 env 唯一、gameId/secret 不可改 |
...
- 创建：忽略前端传入的 gameId/gameSecret/env；落当前 env；建默认市场；写审计。
- alias 唯一：同 env 冲突 409；不同 env 可重复。
- 详情：secret 脱敏（`masked`、`secretMasked=true`）；跨 env 查不到 → 404。
...
1. **`game_id` 生成策略**：假设为「per-env 从 100000 起自增数字串」（依据 `go_domain_api_draft.md` 示例 `100001`）。若需全局唯一/雪花/带前缀，需上游确认。
...
5. **`UNIQUE(env, alias)`**：原 DDL 无 alias 唯一键，本文按业务规则「alias 唯一」补为 `UNIQUE(env, alias)`。若 alias 仅要求展示用途、允许重复，需上游确认去掉。
