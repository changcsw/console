...
```text
Game (games, 业务表/每环境 schema)
  └── GameMarketChannel (game_channels, 业务表/每环境 schema, 按 (game_id_ref, market_code, channel_id_ref) 唯一)   ← `channel` D2：该表本身即渠道实例落地表
        └── ChannelLoginConfig (game_channel_login_configs, 业务表/每环境 schema, 每实例 0..1 条)   ← 本模块
              └─(模板驱动) channel_login_templates (平台级共享 schema platform, 按 (channel_id_ref, template_version))
```
...
- 一个 `game_channel` 最多一条登录配置（`UNIQUE(game_channel_id_ref)`，见 §3）。不存在时视为"未配置"（前端按 `empty` 呈现）。
...
```go
// 渠道登录配置实例（游戏维度业务表，每环境独立 schema，不带 env 列；挂在 GameMarketChannel 上）
...
本模块涉及两张表：`channel_login_templates`（平台级模板，放在共享 schema `platform`，**不带 env**）与 `game_channel_login_configs`（配置实例，游戏维度业务表，**每环境独立 schema、不带 env 列**）。下列以 `migrations/000001_init.up.sql` 真实列为准，并叠加 `00` D1 的 schema-per-env 改造。
...
> 注意：模板表**不加 env**，全环境共享同一套模板定义。该表为**简单模板表**（见 `00` §4.4.1），不走 §3.3 三态机；本模块运行时**只取该渠道 `enabled=TRUE` 的最新 `template_version`**用于渲染与校验（解析逻辑见 §5.2）。

### 3.2 `game_channel_login_configs`（配置实例，游戏维度业务表，每环境独立 schema，不带 env 列 —— 决策 D1）

本表为游戏维度业务表，在每个环境 schema（`develop`/`sandbox`/`production`）各一份同名同结构表，**不带 `env` 列**——行属于哪个 env 由其所在 schema 决定。逐字段如下：

| 列 | 类型 | 默认值 | 约束/说明 |
| --- | --- | --- | --- |
| `id` | BIGSERIAL | — | 主键 |
| `game_channel_id_ref` | BIGINT | — | `NOT NULL REFERENCES game_channels(id)`（同 schema 普通外键）；指向 `GameMarketChannel` 实例（该实例自带 `market_code`） |
...
唯一约束（不再前置 env，schema 内自成体系）：

```text
UNIQUE (game_channel_id_ref)
```

> 解释：每个环境 schema 内自成体系，唯一性天然按 schema 隔离。`game_channels` 唯一键为 `(game_id_ref, market_code, channel_id_ref)`（`channel` D2），即 `game_channel_id_ref` 已隐含 `(game, market, channel)` 全维度。本表只需保证"**同一 schema 内同一渠道实例至多一条登录配置**"，`UNIQUE(game_channel_id_ref)` 即可。

外键（`00` §2.2）：业务表→业务表（`game_channels`）在同一环境 schema 内用普通外键即可。父子行必然同 schema（= 同 env），无需任何 env 一致性校验或复合 env 外键。

### 3.3 迁移片段（v2，每环境 schema 各建一份，示意）

```sql
-- 在每个环境 schema（develop/sandbox/production）下建同名同结构表，不带 env 列
...
> `channel_login_templates` 放在共享 schema `platform`，全环境共享，不带 env。运行时连接设 `search_path = <当前env>, platform`，仓储 SQL 不写 schema 前缀、不带 env 谓词。
...
| 枚举 | 取值 | 默认值 | 本模块用途 |
| --- | --- | --- | --- |
| `Environment` | `develop` / `sandbox` / `production` | `develop` | 决定 `game_channel_login_configs` 落在哪个环境 schema（业务表不带 env 列） |
...
1. 由 `game_channel_id_ref` 反查 `game_channels` 得 `channel_id_ref`，再取该渠道 **`enabled=TRUE` 的最新 `template_version`** 的 `channel_login_templates`（若按 `template_version` 显式指定则取指定版本）。模板不存在 ⇒ 拒绝（`VALIDATION_FAILED`，"该渠道未配置登录模板"）。
...
- 登录配置随 `game_channels`（GameMarketChannel）按 `(game, market, channel)` 独立存在（env 维度由所在 schema 体现，每环境各一份物理行）。`CN / huawei_cn` 的登录配置与其它 market 下的同渠道实例**互不影响**：各自的 `config_json`、密钥、文件、`config_status`、`enabled` 全部独立。
...
统一前缀 `/api/admin`；除登录类外均需 `Authorization: Bearer <accessToken>`；请求/响应 JSON 字段 camelCase；写操作落**当前运行环境对应 schema**（不接受前端指定/跨 schema 写）。本模块两个接口：
...
- 鉴权权限码：`channel.read`。
- 行为：
  - 校验 `gameChannelId` 在当前环境 schema 存在；不存在 ⇒ 404 `NOT_FOUND`。
...
| `templateVersion` | string | 否 | 最新 `enabled` 版本 | 指定按哪个模板版本校验；缺省取 `enabled=TRUE` 的最新 `template_version` |
...
| `NOT_FOUND` | 404 | `gameChannelId` 在当前环境 schema 不存在 |
| `VALIDATION_FAILED` | 400 | 非 channel_only 渠道 / 未知字段 / 必填或规则未过 / 无登录模板 |
| `CONFLICT` | 409 | 并发写导致唯一键冲突（`(game_channel_id_ref)`） |
...
- `GetLoginConfig(ctx, env, gameChannelID)`：取渠道实例 + 校验 `channel_only` + 取最新 `enabled` 模板 + 取/构造配置实例 + 脱敏 ⇒ 组装 GET 响应。
- `UpsertLoginConfig(ctx, env, gameChannelID, cmd)`：校验 `channel_only` → 取模板 → 模板驱动校验（§5.2）→ 密文加密/文件引用化（含 `"******"` 保留原密文逻辑）→ 推导 `config_status`/`last_check_*`（§5.3）→ upsert（`UNIQUE(game_channel_id_ref)`，当前环境 schema 内）→ 写审计 → 返回脱敏结果。
...
- `ChannelLoginTemplateRepository`（读 `channel_login_templates`：按 `channel_id_ref` 取 `enabled=TRUE` 的最新 `template_version`）。
- `ChannelLoginConfigRepository`（`game_channel_login_configs` 的 Get/Upsert；SQL 不写 schema 前缀、不带 env 谓词，环境由连接 `search_path` 决定）。
...
type ChannelLoginConfigRepository interface {
    GetByGameChannel(ctx context.Context, gameChannelID int64) (*ChannelLoginConfig, error) // 不存在返回 (nil, nil)；当前环境 schema 内
    Upsert(ctx context.Context, cfg *ChannelLoginConfig) error // 按 (game_channel_id_ref) upsert（当前环境 schema 内，环境由 search_path 决定）
}
...
> 维度定义见 `03-testing.md §4`（S1 成功 / S2 鉴权401 / S3 权限403 / S4 校验失败 / S5 冲突 / S6 跨env（schema 隔离）：写落当前环境 schema、不允许跨 schema 写 / S7 审计 / S8 脱敏 / S9 分页 / S10 事务回滚）。`✓`=覆盖，`—`=不适用。后端 manifest：`tests/backend/scenarios/channel-login.yaml`；前端 e2e：`tests/frontend/e2e/channels.spec.ts`（渠道登录页签）。
...
| PUT /api/admin/game-channels/{gameChannelId}/login-config | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | — | ✓ | 仅 channel_only 渠道可写（否则 VALIDATION_FAILED）；与 account_system 隔离；secret 脱敏（S8）；config_status 后端推导；UNIQUE(game_channel_id_ref) 冲突→CONFLICT；复制创建强制 invalid；隐藏/不兼容不进快照/同步；upsert+审计同事务回滚 |
...
- 每环境 schema 下 `game_channel_login_configs` 不带 `env` 列、唯一键为 `(game_channel_id_ref)`；同一 schema 内同 `game_channel_id_ref` 重复 upsert 不产生重复行。
- 不同环境 schema 下同名表各存一条（验证 schema 隔离）。
- `channel_login_templates` 无 env 列（平台级，共享 schema `platform`）。
...
3. **模板版本运行时取 `enabled=TRUE` 的最新 `template_version`**（简单模板表，不走 §3.3 三态机）；`templateVersion` 入参仅用于显式指定校验版本（少见）。模板版本本身的维护不在本模块。
