...
| 鉴权（`auth`） | `preview` 挂 `sync.preview`，`execute` 挂 `sync.execute`；详见 §6。 |

### 1.4 锁定决策回链（本模块必须贯彻）

| 决策 | 在本模块的落地 |
| --- | --- |
| D1 同库跨 schema diff | 同一物理库内跨 schema 比对（`sandbox.*` ↔ `production.*`）；`source_env=sandbox`、`target_env=production` 即源/目标 schema 名，不跨库。 |
...
BaselineToken（解码后的逻辑结构）
  gameId            string
  syncJobId         string      // preview 落库的 sync_jobs.id；execute 据此原地更新同一行（见 §3.1/§5.8）
  sourceEnv         "sandbox"
  targetEnv         "production"
...
> 本节按真实列逐字段说明，并补充本模块的语义与约束。两表均为**平台级任务记录表**，位于共享 `platform` schema（不带 `env` 列），其环境维度通过 `source_env` / `target_env` 字段显式表达（值即源/目标 schema 名；`00` §2.2：sync 域是唯一允许同时访问两个环境 schema 的域）。
...
| `game_id_ref` | VARCHAR(64) | — | NOT NULL | 关联**逻辑游戏的业务 `game_id`（字符串）**。因 `games` 现按环境分布在多个 schema、各自有独立自增 `id`，平台级 `sync_jobs` 不能用数值 FK 指向某一 schema 的 `games(id)`，故以**业务 `game_id` 标识逻辑游戏**，不设跨 schema FK；存在性由应用层对 `sandbox.games` 校验（见下）。 |
...
- **生命周期（审定）**：一次 preview 生成一行 `previewed` 记录（`sync_job_items.applied=false`）；execute **更新同一行** `sync_job_id` 到 `succeeded/failed`（写回 `target_hash_after`/`executed_at`、`MarkItemsApplied`），**不另建行**。`baseline_token` 内含 `syncJobId`，execute 据此定位并更新该行（见 §11 Q3）。
- **跨 schema 引用（`00` §2.2 / `01` §4.3）**：`sync_jobs` 位于 `platform` schema，是同步任务流水，**不带 env**（用 `source_env`/`target_env` 表达方向）；它以业务 `game_id` 标识逻辑游戏，**不设跨 schema 数值 FK**（`games` 已按环境分多 schema，各 schema 自增 id 不通用）。逻辑游戏存在性与 `source_env=sandbox`/`target_env=production` 的合法性由**应用层校验**保证（仓储写入接收 `ctx`，校验对应 `sandbox.games` 行存在）。
...
| `entity_type` | VARCHAR(64) | — | NOT NULL | 实体类型（如 `product` / `game_channel` / `channel_package` / `game_channel_plugin_config` / `channel_package_plugin_override` / `payment_route` / `config_snapshot`）。 |
...
| `channels` | 渠道实例（按 market 拆分） | `game_channels`（+ `game_channel_login_configs` / `game_channel_iap_configs` / `game_channel_plugin_configs`） |
| `packages` | 渠道包 | `channel_packages`（+ `channel_package_iap_overrides` / `channel_package_plugin_overrides`） |
...
```text
1. 运营在 sandbox 完成各 section 配置（写入 sandbox schema）
...
**数据采集**：对每个 section，分别从 `sandbox` schema（源）与 `production` schema（目标）加载该 game 的有效记录（用 schema 限定名跨 schema 读）。「有效」判定：

- 渠道实例（channels/packages 下游）：`hidden=false` 且 与当前 market 兼容 且 `config_status == 'valid'`（与 `00 §9`、`channel §4`、`snapshot §5.1` 口径一致：隐藏 / 不兼容 / 非 valid 实例一律排除）。
...
- upsert 写到 `production` schema（schema 限定名），以各表业务唯一键为冲突键（业务表唯一键不含 env，D1）：存在则更新有变化字段，不存在则插入。
...
| 接口 | 权限码 | 是否写审计 |
| --- | --- | --- |
| `POST .../sync/preview` | `sync.preview` | 否（预览只读，可选记录轻量日志） |
| `POST .../sync/execute` | `sync.execute` | **是**（`action=sync.execute`） |
| `GET .../sync-jobs` | `sync.preview` | 否 |
...
Preview(ctx, gameID, req) -> (SyncPreview, error)
  ...
  2. 对每个 section：loadEffective(schema=sandbox) / loadEffective(schema=production)  // 跨 schema 读
  3. diff(sandboxSet, productionSet) -> []DiffChange（脱敏 secret）
  4. canonicalize+hash -> sourceHash / targetHashBefore
  5. syncJobId = repo.CreateJob(status=previewed); repo.AddItems(applied=false)
...
Execute(ctx, gameID, req) -> (SyncJobResult, error)
  ...
  // —— 审定执行顺序：nonce 去重 → baseline hash 复核 → 依赖拓扑校验 → 单事务有序 upsert ——
  3. nonce 去重（幂等闸门）：if nonceAlreadyConsumed(token.nonce) -> SYNC_TOKEN_CONSUMED(409)
  4. baseline hash 复核：targetHashNow = hash(loadEffective(schema=production))
     if targetHashNow != token.targetHashBefore -> SYNC_BASELINE_MISMATCH
  5. 依赖拓扑校验：checkDependencies(selectedSections, productionState)  // 缺失 -> VALIDATION_FAILED(details)
  6. BEGIN TX（单事务有序 upsert；原地更新 token.syncJobId 对应的 previewed 行）
...
| 鉴权（`00` §7.5 / D5） | `preview→sync.preview`、`execute→sync.execute`；危险操作必挂权限。 |
| env 模型（`00` §2 / D1） | 同库跨 schema diff（`sandbox.*`↔`production.*`），仅 sync 域允许显式 `source_env/target_env`（即源/目标 schema）。 |
...
| `POST .../sync/execute` | ✓ | ✓ | ✓ | ✓(UNKNOWN_SECTION/token非法) | ✓(BASELINE_MISMATCH/TOKEN_CONSUMED) | ✓(跨 schema sandbox→production) | ✓(sync.execute) | ✓ | — | ✓(全程单事务,不部分写入) | **基线 hash 复核(D6)、nonce 去重幂等、依赖拓扑校验、删除 opt-in、显式 selected_sections** |
...
| Q3 | 已定 | preview 生成的 `sync_jobs(previewed)` 与 execute 任务是否同一行。 | **审定：同一行**。preview 建 `previewed` 行（items `applied=false`），execute **原地更新**该行到 `succeeded/failed`，不另建行；`baseline_token` 内含 `syncJobId`，execute 据此定位（见 §3.1）。 |
